...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['updateUserDetailsButton'])) {
  ...
  $validLogOnStatuses = array('fullAdmin', 'pageEditor', 'staff');
  ...
  if ($inputOK === true) {
    ...
    if ($stmt->execute()) {
      ...
      if ($passwordUpdate === true) {
        ...
        $updatePasswordQuery = "UPDATE UsersDB SET UsersPassword = ? WHERE UsersID = ?";
        $stmtPwd = $connection->prepare($updatePasswordQuery);
        $stmtPwd->bind_param("si", $hashedPassword, $userToEditID);
        
        if ($stmtPwd->execute()) {
          $feedbackMessage = "<p class=\"formFeedbackSuccess\">✓ User details and password updated successfully.</p>";
        } else {
          $feedbackMessage .= "<p class=\"formFeedbackError\">User details updated but password update failed: " . $stmtPwd->error . "</p>";
        }
        $stmtPwd->close();
      }
    } else {
  ...
  $selectQuery = "SELECT * FROM UsersDB WHERE UsersID = ?";
