...
// Get filter options
$logOnStatuses = mysqli_query(
	$connection,
	"SELECT DISTINCT LogOnStatus FROM users_tb WHERE LogOnStatus IS NOT NULL AND LogOnStatus != '' ORDER BY LogOnStatus",
);
$schoolStatuses = mysqli_query(
	$connection,
	"SELECT DISTINCT SchoolStatus FROM users_tb WHERE SchoolStatus IS NOT NULL AND SchoolStatus != '' ORDER BY SchoolStatus",
);
$classes = mysqli_query($connection, "SELECT ClassID, classname FROM classes_tb ORDER BY classOrder, classname");
...
$usersQuery = "SELECT u.*, c.classname, c.classOrder 
               FROM users_tb u 
               LEFT JOIN classes_tb c ON u.AssociatedClassID = c.ClassID 
               WHERE 1=1";
...
$filterParams = "";
if (!empty($filterLogOnStatus)) {
	$filterParams .= "&filterLogOnStatus=" . urlencode($filterLogOnStatus);
}
if (!empty($filterSchoolStatus)) {
	$filterParams .= "&filterSchoolStatus=" . urlencode($filterSchoolStatus);
}
if ($filterClass > 0) {
	$filterParams .= "&filterClass=" . $filterClass;
}
