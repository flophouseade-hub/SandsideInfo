...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['updateTasksButton'])) {
  $completedTasks = $_POST['completedTasks'] ?? array();
  ...
  try {
    ...
    $updateQuery = "UPDATE UserTasksDB SET UserSetTaskComplete = ? WHERE UTUsersID = ? AND UserSetTaskID = ?";
...
$queryUser = "SELECT FirstName, LastName, Email, SchoolStatus FROM UsersDB WHERE UsersID = ?";
...
// Get all courses assigned to this user (distinct courses from UserTasksDB)
...
foreach ($userCoursesArray as $userCourse) {
  ...
  if ($resultCourse->num_rows > 0) {
    ...
    $queryTasks = "SELECT ut.UserSetTaskID, ut.UserSetTaskComplete, ut.UserSetCourseDate,
                          ...
                          rl.LRLink, rl.LRName
                   FROM UserTasksDB ut
                   JOIN TasksDB t ON ut.UserSetTaskID = t.TaskID
                   LEFT JOIN ResourceLibrary rl ON t.TaskResource = rl.LinkedResourceID
  ...
  foreach ($coursesWithTasksArray as $course) {
    ...
    if (count($tasks) > 0) {
      foreach ($tasks as $task) {
        ...
        $resourceTitle = !empty($task['ResourceTitle']) ? htmlspecialchars($task['ResourceTitle'], ENT_QUOTES, 'UTF-8') : 'View Resource';
          ...
          <div class=\"task-card-inner\">
            <div class=\"task-card-row\">
              ...
              <div class=\"task-content\">
                <label class=\"task-label" . ($isComplete == 1 ? " completed" : "") . "\">
      ...
      $coursesHTML .= "<p style=\"color: #666; font-style: italic; padding: 15px;\">No tasks assigned for this course.</p>";
    }
