...
// Get filter options
$errorTypes = mysqli_query(
	$connection,
	"SELECT DISTINCT ErrorType FROM error_log_tb WHERE ErrorType IS NOT NULL AND ErrorType != '' ORDER BY ErrorType",
);
$errorUsers = mysqli_query(
	$connection,
	"SELECT DISTINCT e.UserID, u.FirstName, u.LastName 
                                          FROM error_log_tb e 
                                          LEFT JOIN users_tb u ON e.UserID = u.UsersID 
...
switch ($sortBy) {
	...
	case "user":
		$errorsQuery .=
			" ORDER BY u.LastName " . ($sortOrder === "ASC" ? "ASC" : "DESC") . ", u.FirstName, e.ErrorTime DESC";
		break;
	default:
...
$countQuery = "SELECT COUNT(*) as total FROM error_log_tb WHERE 1=1";
if (!empty($filterType)) {
...
if (!empty($filterType) || $filterUser > 0) {
	...
	print "<button type='button' onclick=\"location.href='viewerror_log_tbPage.php{$sortParam}{$limitParam}'\" style='padding: 4px 12px; background-color: #666; color: white; border: none; border-radius: 3px; cursor: pointer; font-size: 12px; font-weight: 500;'>Clear Filters</button>";
}
print "</td>
  </tr>
</thead>";

print "<form id='filterForm' method='GET' action='viewerror_log_tbPage.php' style='display: none;'>";
if ($sortBy !== "time") {
