...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['editPageDetailsButton'])) {
  ...
  if (!empty($editPageColour)) {
    if (preg_match('/^#[0-9A-Fa-f]{6}$/', $editPageColour)) {
      ...
      // Valid rgb color
    } elseif (in_array(strtolower($editPageColour), ['red', 'blue', 'green', 'yellow', 'orange', 'purple', 'pink', 'brown', 'black', 'white', 'gray', 'grey'])) {
      // Valid color name
    } else {
      $inputOK = false;
      $feedbackMessage .= "<p>Page Colour must be a valid hex code with # (e.g., #FF5733), rgb value (e.g., rgb(255,87,51)), or color name (e.g., red).</p>";
    }
  }
  ...
  if ($editPageType != 'builtInPage' && !empty($editPageContentRefs)) {
    ...
    if ($validatePageContentRefs != true) {
      ...
      $inputOK = false;
    } else if ($editPageType == 'blockMenu') {
    ...
    } else if ($editPageType == 'sectionsPage') {
    ...
    if ($stmt->execute()) {
      ...
      if ($editPageType == 'sectionsPage' && isset($_POST['selectedSections'])) {
        ...
        if (!empty($selectedSections)) {
          ...
          $insertQuery = "INSERT INTO PageSectionsDB (PSPageID, PSSectionID, PSDisplayOrder, PSIsActive, PSShowTitle) VALUES (?, ?, ?, 1, 1)";
        ...
        $_SESSION['pageSections'][$pageToEditID] = array();
        if (!empty($selectedSections)) {
          $sectionArray = explode(',', $selectedSections);
          foreach ($sectionArray as $index => $sectionID) {
            $sectionID = trim($sectionID);
      ...
      if ($editPageType == 'blockMenu' && isset($_POST['selectedPages'])) {
        ...
        if (!empty($selectedPages)) {
          ...
          $insertQuery = "INSERT INTO PageMenuLinksDB (PMLMenuPageID, PMLLinkedPageID, PMLDisplayOrder, PMLIsActive) VALUES (?, ?, ?, 1)";
          $insertStmt = $connection->prepare($insertQuery);
        ...
        $_SESSION['pageMenuLinks'][$pageToEditID] = array();
        if (!empty($selectedPages)) {
          $pageArray = explode(',', $selectedPages);
          foreach ($pageArray as $index => $linkedPageID) {
            $linkedPageID = trim($linkedPageID);
      ...
      $feedbackMessage = "<p style=\"color: #dc3545; font-weight: bold;\">✗ Error updating page details: " . htmlspecialchars($stmt->error, ENT_QUOTES, 'UTF-8') . "</p>";
    }
...
$groupColoursWithPages = array();
if (!empty($editPageGroup)) {
    $colourQuery = "SELECT PageColour, PageName FROM PagesOnSite WHERE PageGroup = ? AND PageColour IS NOT NULL AND PageColour != '' ORDER BY PageColour ASC, PageName ASC";
...
switch ($editPageType) {
  case 'blockMenu':
    $formInstruction = "You are editing a Block Menu Page. Block Menu pages display a menu of blocks that link to other pages or menus.";
    ...
    break;
  case 'sectionsPage':
    $formInstruction = "You are editing a Sections Page. Sections pages display content sections that can include text, images, and other media.";
    $buttonText = "Update This Page";
    ...
    break;
  case 'builtInPage':
    $formInstruction = "This is a built-in page. Built-in pages have special functions and only some of their content can be edited.";
...
$formAndContentString .= "
      ...
      <div id=\"imagePreviewContainer\" style=\"margin-top: 10px; display: " . (!empty($editPageImageIDRef) ? "block" : "none") . ";\">
        <img id=\"imagePreview\" src=\"" . (!empty($editPageImageIDRef) && isset($_SESSION['imageLibrary'][$editPageImageIDRef]) ? htmlspecialchars($_SESSION['imageLibrary'][$editPageImageIDRef]['ImageLink'], ENT_QUOTES, 'UTF-8') : "") . "\" 
...
$formAndContentString .= "
    ...
    <div class=\"formField\">
      <label style=\"display: flex; align-items: center; gap: 10px; cursor: pointer;\">
        <input type=\"checkbox\" name=\"fvPageMakerEditOnly\" value=\"1\" " . ($editPageMakerEditOnly == 1 ? 'checked' : '') . " 
...
if ($showContentRefs && $editPageType == 'blockMenu') {
  ...
  if (count($currentMenuLinks) > 0) {
    ...
    foreach ($currentMenuLinks as $menuLink) {
      ...
      $linkedPageName = isset($_SESSION['pagesOnSite'][$linkedPageID]) ? htmlspecialchars($_SESSION['pagesOnSite'][$linkedPageID]['PageName'], ENT_QUOTES, 'UTF-8') : 'Unknown Page';
      ...
      $menuLinksList .= "<span style='color: #666; font-size: 12px;'>Order: " . ($menuLink['DisplayOrder'] + 1) . "</span>";
      $menuLinksList .= "</li>";
    }
    $menuLinksList .= "</ul>";
  } else {
    $menuLinksList = "<p style='color: #666; font-style: italic; margin: 10px 0;'>No pages currently in this menu. Use the checkboxes below to add pages.</p>";
  }
...
} else if ($showContentRefs && $editPageType == 'sectionsPage') {
  ...
  if (count($currentSections) > 0) {
    ...
    foreach ($currentSections as $section) {
      ...
      $sectionTitle = isset($_SESSION['sectionDB'][$sectionID]) ? htmlspecialchars($_SESSION['sectionDB'][$sectionID]['SectionTitle'], ENT_QUOTES, 'UTF-8') : 'Unknown Section';
      ...
      $sectionsList .= "<span style='color: #666; font-size: 12px;'>Order: " . ($section['DisplayOrder'] + 1) . "</span>";
      $sectionsList .= "</li>";
    }
    $sectionsList .= "</ul>";
  } else {
    $sectionsList = "<p style='color: #666; font-style: italic; margin: 10px 0;'>No sections currently assigned to this page. Use the checkboxes below to add sections.</p>";
  }
...
if ($editPageType == 'blockMenu') {
        ...
        <table style=\"width: 100%;\">
          <thead>
            <tr>
              ...
              <th><a href=\"editPageDetailsPage.php{$baseParams}&sortBy=id" . ($filterType ? "&filterType=" . urlencode($filterType) : "") . ($filterGroup ? "&filterGroup=" . urlencode($filterGroup) : "") . "#pagesReferenceTable\" style=\"text-decoration: none; color: inherit;\">ID</a></th>
              <th><a href=\"editPageDetailsPage.php{$baseParams}&sortBy=name" . ($filterType ? "&filterType=" . urlencode($filterType) : "") . ($filterGroup ? "&filterGroup=" . urlencode($filterGroup) : "") . "#pagesReferenceTable\" style=\"text-decoration: none; color: inherit;\">Page Name</a></th>
              <th><a href=\"editPageDetailsPage.php{$baseParams}&sortBy=description" . ($filterType ? "&filterType=" . urlencode($filterType) : "") . ($filterGroup ? "&filterGroup=" . urlencode($filterGroup) : "") . "#pagesReferenceTable\" style=\"text-decoration: none; color: inherit;\">Description</a></th>
              <th>Type</th>
              <th>Group</th>
            </tr>
            <tr>
  ...
  if ($filterType || $filterGroup) {
    print("
                <button type=\"button\" class=\"clearFilterButton\" onclick=\"location.href='editPageDetailsPage.php{$baseParams}" . ($sortBy ? "&sortBy=" . urlencode($sortBy) : "") . "#pagesReferenceTable'\">Clear</button>");
  }
  ...
  print("
              ...
              <td>
                <form method=\"get\" action=\"editPageDetailsPage.php#pagesReferenceTable\" style=\"display: inline;\">
                  <input type=\"hidden\" name=\"editPageID\" value=\"" . htmlspecialchars($currentPageID, ENT_QUOTES, 'UTF-8') . "\">
                  " . ($sortBy ? "<input type=\"hidden\" name=\"sortBy\" value=\"" . htmlspecialchars($sortBy, ENT_QUOTES, 'UTF-8') . "\">" : "") . "
                  <select name=\"filterType\" class=\"filterSelect\" onchange=\"this.form.submit()\">
                    <option value=\"\">All Types</option>");
                    
  foreach ($pageTypes as $type) {
    $selected = ($type === $filterType) ? ' selected' : '';
    print("<option value=\"" . htmlspecialchars($type, ENT_QUOTES, 'UTF-8') . "\"$selected>" . htmlspecialchars($type, ENT_QUOTES, 'UTF-8') . "</option>");
  }
  ...
  print("
              ...
              <td>
                <form method=\"get\" action=\"editPageDetailsPage.php#pagesReferenceTable\" style=\"display: inline;\">
                  <input type=\"hidden\" name=\"editPageID\" value=\"" . htmlspecialchars($currentPageID, ENT_QUOTES, 'UTF-8') . "\">" . ($sortBy ? "<input type=\"hidden\" name=\"sortBy\" value=\"" . htmlspecialchars($sortBy, ENT_QUOTES, 'UTF-8') . "\">" : "") . ($filterType ? "<input type=\"hidden\" name=\"filterType\" value=\"" . htmlspecialchars($filterType, ENT_QUOTES, 'UTF-8') . "\">" : "") . "
                  <select name=\"filterGroup\" class=\"filterSelect\" onchange=\"this.form.submit()\">
                    <option value=\"\">All Groups</option>");
                    
  foreach ($pageGroups as $group) {
    $selected = ($group === $filterGroup) ? ' selected' : '';
    print("<option value=\"" . htmlspecialchars($group, ENT_QUOTES, 'UTF-8') . "\"$selected>" . htmlspecialchars($group, ENT_QUOTES, 'UTF-8') . "</option>");
  }
  ...
  foreach ($filteredPages as $pageID => $pageDetails) {
    ...
    $pageGroup = isset($pageDetails['PageGroup']) ? htmlspecialchars($pageDetails['PageGroup'], ENT_QUOTES, 'UTF-8') : '';
...
if ($editPageType == 'sectionsPage') {
  ...
  foreach ($_SESSION['sectionDB'] as $sectionID => $sectionDetails) {
    // Apply group filter
    if ($filterSectionGroup !== '' && (!isset($sectionDetails['SectionGroup']) || $sectionDetails['SectionGroup'] !== $filterSectionGroup)) {
      continue;
    }
  ...
  print("
        <table style=\"width: 100%;\">
          <thead>
            <tr>
              <th class=\"listAllTableCellCenter\">Select</th>
              <th><a href=\"editPageDetailsPage.php{$baseParams}&sortBySection=id" . ($filterSectionGroup ? "&filterSectionGroup=" . urlencode($filterSectionGroup) : "") . "#sectionsReferenceTable\" style=\"text-decoration: none; color: inherit;\">Section ID</a></th>
              <th><a href=\"editPageDetailsPage.php{$baseParams}&sortBySection=title" . ($filterSectionGroup ? "&filterSectionGroup=" . urlencode($filterSectionGroup) : "") . "#sectionsReferenceTable\" style=\"text-decoration: none; color: inherit;\">Section Title</a></th>
              <th><a href=\"editPageDetailsPage.php{$baseParams}&sortBySection=content" . ($filterSectionGroup ? "&filterSectionGroup=" . urlencode($filterSectionGroup) : "") . "#sectionsReferenceTable\" style=\"text-decoration: none; color: inherit;\">Content Preview</a></th>
              <th><a href=\"editPageDetailsPage.php{$baseParams}&sortBySection=group" . ($filterSectionGroup ? "&filterSectionGroup=" . urlencode($filterSectionGroup) : "") . "#sectionsReferenceTable\" style=\"text-decoration: none; color: inherit;\">Group</a></th>
            </tr>
            <tr>
              <td></td>
              <td></td>
  ...
  if ($filterSectionGroup) {
    print("
                <button type=\"button\" class=\"clearFilterButton\" onclick=\"location.href='editPageDetailsPage.php{$baseParams}" . ($sortBySection ? "&sortBySection=" . urlencode($sortBySection) : "") . "#sectionsReferenceTable'\">Clear</button>");
  }
  ...
  print("
              ...
              <td>
                <form method=\"get\" action=\"editPageDetailsPage.php#sectionsReferenceTable\" style=\"display: inline;\">
                  <input type=\"hidden\" name=\"editPageID\" value=\"" . htmlspecialchars($currentPageID, ENT_QUOTES, 'UTF-8') . "\">
                  " . ($sortBySection ? "<input type=\"hidden\" name=\"sortBySection\" value=\"" . htmlspecialchars($sortBySection, ENT_QUOTES, 'UTF-8') . "\">" : "") . "
  ...
  foreach ($filteredSections as $sectionID => $sectionDetails) {
    ...
    $sectionGroup = isset($sectionDetails['SectionGroup']) ? htmlspecialchars($sectionDetails['SectionGroup'], ENT_QUOTES, 'UTF-8') : '';
