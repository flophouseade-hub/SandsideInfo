...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["deletePageButton"])) {
	...
	$protectedPageIDs = [1, 15]; // 1 = Main Menu, 15 = Admin Home (adjust IDs as needed)

	// Get page name for error messages
	$pageToDeleteName = isset($_SESSION["pagesOnSite"][$pageToDeleteID])
		? $_SESSION["pagesOnSite"][$pageToDeleteID]["PageName"]
		: "Unknown";
		...
		if (count($referencingLocalMenuPages) > 0) {
			...
			if (count($referencingPages) > 0) {
				...
				if ($stmt->execute()) {
					...
					$inputOK = false;
					$feedbackMessage =
						"<p style=\"color:red; font-weight: bold; margin-left: 20px;\">Database error deleting page: $pageToDeleteName (ID: $pageToDeleteID). " .
						$stmt->error .
						"</p>";
				}
...
if (!empty($filterGroup)) {
	if ($filterGroup === "_none_") {
		...
		$titleHTML .=
			" <span style=\"color: #1976d2;\">- Group: " .
			htmlspecialchars($filterGroup, ENT_QUOTES, "UTF-8") .
			"</span>";
	}
}
...
$protectedPageIDs = [1, 15]; // 1 = Main Menu, 15 = Admin Home
...
if (!empty($filterAccess)) {
	$filterParams .= "&filterAccess=" . urlencode($filterAccess);
}
if (!empty($filterGroup)) {
...
foreach ($pages_on_site_tb as $pageID => $pageDetails) {
	...
	$protectedPageIDs = [1, 15, $thisPageID]; // Main Menu, Admin Home, and current page
