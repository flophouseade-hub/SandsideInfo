...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["registerButton"])) {
	...
	if ($inputRegistrationCode !== "The Lodge Cafe") {
		$feedbackMessage .=
			"<p class=\"formFeedbackError\">Invalid registration code. Please contact an administrator to obtain the correct code.</p>";
		$inputError = true;
	}
	...
	if ($inputError === false) {
		...
		if ($result->num_rows > 0) {
			$inputError = true;
			$feedbackMessage .=
				"<p class=\"formFeedbackError\">This email address is already registered. Please use the login page or reset your password.</p>";
		} else {
			...
			if ($stmtInsert->execute()) {
				...
				$message .=
					"To complete your registration and set your password, please visit the password reset page:\n";
