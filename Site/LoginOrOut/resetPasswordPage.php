...
if (!empty($token)) {
  ...
  $query = "SELECT * FROM UsersDB WHERE PasswordResetToken = ? AND PRTokenExpiry >= ?";
  ...
  if ($stmt->execute()) {
    ...
    if ($result->num_rows > 0) {
      ...
      $tokenValid = false;
      $feedbackMessage = "<p class=\"formFeedbackError\">Invalid or expired token. Please request a new password reset.</p>";
    }
    $stmt->close();
  } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['setNewPasswordButton'])) {
  ...
  if ($inputError === false) {
    ...
    $updateQuery = "UPDATE UsersDB SET UsersPassword = ?, PasswordResetToken = NULL, PRTokenExpiry = NULL WHERE Email = ?";
    $stmtUpdate = $connection->prepare($updateQuery);
    $stmtUpdate->bind_param("ss", $hashedPassword, $editUserEmail);

    if ($stmtUpdate->execute()) {
      $passwordResetSuccess = true;
      $feedbackMessage = "<p class=\"formFeedbackSuccess\">✓ Your password has been reset successfully. You can now log in with your new password.</p>";
      $inputError = false;
    } else {
