...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["resetPasswordButton"])) {
	...
	if ($inputError === false) {
		...
		$stmt = $connection->prepare(
			"SELECT UsersID, FirstName, LastName, LogOnStatus, UsersPassword FROM users_tb WHERE Email = ?",
		);
		...
		if ($numberOfRows > 1) {
			$inputError = true;
			$feedbackMessage .=
				"<p class=\"formFeedbackError\"><strong>Error: Multiple users found with that email address.</strong></p><p class=\"formFeedbackError\">Please contact the administrator.</p>";
		} elseif ($numberOfRows < 1) {
			$inputError = true;
			$feedbackMessage .=
				"<p class=\"formFeedbackError\"><strong>Error: Email not found.</strong></p><p class=\"formFeedbackError\">Did you register with your school email address or your personal email?</p>";
		} elseif ($numberOfRows == 1) {
			...
			$inputError = true;
			$feedbackMessage .=
				"<p class=\"formFeedbackError\"><strong>Error: There is an unexpected issue with your data. </strong></p><p class=\"formFeedbackError\">Please contact the administrator.</p>";
		}
		...
		if ($inputError === false) {
			...
			$updateQuery = "UPDATE users_tb SET PasswordResetToken = ?, PRTokenExpiry = ? WHERE Email = ?";
			...
			if ($stmtUpdate->execute()) {
				...
				if (mail($to, $subject, $message, $headers)) {
					...
					$inputError = true;
					$feedbackMessage .=
						"<p class=\"formFeedbackError\">Error: Could not send email. Please try again later or contact the administrator.</p>";
				}
			} else {
