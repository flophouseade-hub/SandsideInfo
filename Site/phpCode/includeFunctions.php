...
function logError($errorType, $errorMessage, $fileName = "", $lineNumber = 0, $userId = null)
{
	...
	if ($conn && !$conn->connect_error) {
		$stmt = $conn->prepare(
			"INSERT INTO ErrorLog (ErrorTime, ErrorType, ErrorMessage, FileName, LineNumber, UserID) VALUES (NOW(), ?, ?, ?, ?, ?)",
		);
		if ($stmt) {
...
function findRefStringPositionsInContentString($contentString, $startRefString, $endRefString)
{
	...
	if ($startPos === false) {
		...
		if ($endPos === false) {
			...
			$returnArray[2] = substr(
				$contentString,
				$startPos + strlen($startRefString),
				$endPos - strlen($startRefString) - $startPos,
			);
			return $returnArray;
		}
	}
...
function generateColorVariations($hexColor, $lightenPercent = 90)
{
	...
	$lighterColor =
		"#" .
		str_pad(dechex($lightR), 2, "0", STR_PAD_LEFT) .
	...
	$splitComp1Lighter =
		"#" .
		str_pad(dechex($splitComp1LightR), 2, "0", STR_PAD_LEFT) .
		str_pad(dechex($splitComp1LightG), 2, "0", STR_PAD_LEFT) .
		str_pad(dechex($splitComp1LightB), 2, "0", STR_PAD_LEFT);

	$splitComp2Lighter =
		"#" .
		str_pad(dechex($splitComp2LightR), 2, "0", STR_PAD_LEFT) .
