...
foreach ($pageMenuLinks as $menuLink) {
    ...
    if ($thisPageAccessLevel == "none" || $thisPageAccessLevel == "allUsers") {
        $hasAccess = true;
    } else if (!isset($_SESSION['currentUserID']) && $thisPageAccessLevel == "notLoggedIn") {
        $hasAccess = true;
    } else if (accessLevelCheck($thisPageAccessLevel) === true && isset($_SESSION['currentUserID']) && $thisPageAccessLevel != "notLoggedIn") {
        $hasAccess = true;
    }
    if ($hasAccess) {
