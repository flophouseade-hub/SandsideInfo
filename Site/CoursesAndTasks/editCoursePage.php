...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["updateCourseButton"])) {
	...
	if (!empty($inputCourseColour)) {
		if (!validateColourCode($inputCourseColour)) {
			$feedbackMessage .=
				"<p style=\"color:red;\">Course Colour must be a valid hex color code (e.g., #808080).</p>";
			$inputError = true;
		}
	}
	...
	if ($inputError === false) {
		...
		if ($resultCheck->num_rows > 0) {
			...
			$updateQuery =
				"UPDATE courses_tb SET CourseName = ?, CourseDescription = ?, CourseGroup = ?, CourseColour = ?, CourseEditBy = ?, CourseEditTime = ? WHERE CourseID = ?";
			...
			if ($stmt->execute()) {
				$feedbackMessage =
					"<p style=\"color: green;\"><strong>Course details updated successfully.</strong></p>";
				...
				$feedbackMessage =
					"<p style=\"color: red;\"><strong>ERROR: Could not update course details: " .
					$stmt->error .
					"</strong></p>";
				$inputError = true;
			}
...
$query =
	"SELECT CourseName, CourseDescription, CourseGroup, CourseColour, CourseMadeBy, CourseMadeTime, CourseEditBy, CourseEditTime FROM courses_tb WHERE CourseID = ?";
...
if (isset($inputError) && $inputError == false && !empty($feedbackMessage)) {
	//$feedbackMessage = "<div class=\"formFeedback\">$feedbackMessage</div>";
} elseif (isset($inputError) && $inputError == true) {
	$feedbackMessage = "<div class=\"formFeedback\">
...
if (count($courseTasksArray) > 0) {
	$tasksListHTML =
		"<h4 style=\"margin-top: 0; margin-bottom: 15px;\">Tasks in this Course (" .
		count($courseTasksArray) .
		")</h4>";
	...
	$tasksListHTML =
		"<p style=\"color: #666; font-style: italic;\">No tasks are currently associated with this course.</p>";
}

$formAndContentString = "<div class=\"formPageWrapper\">";

// Display feedback message
if (!empty($feedbackMessage)) {
	$formAndContentString .= "<div class=\"formBlueInfoBox\">$feedbackMessage</div>";
}
