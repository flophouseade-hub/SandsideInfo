...
while ($courseRow = mysqli_fetch_assoc($coursesResult)) {
    ...
    // Fetch users enrolled in this course with their progress calculated from UserTasksDB
    $enrollmentQuery = "SELECT DISTINCT 
                        ...
                        FROM UserTasksDB ut
                        JOIN UsersDB u ON ut.UTUsersID = u.UsersID
    ...
    $courses[$courseID]['enrollments'] = array();
    while ($enrollmentRow = $enrollmentResult->fetch_assoc()) {
        ...
        $isCompleted = ($enrollmentRow['TotalTasks'] > 0 && $enrollmentRow['CompletedTasks'] === $enrollmentRow['TotalTasks']);
...
if (count($courses) === 0) {
    ...
    foreach ($courses as $courseID => $course) {
        ...
        $courseGroup = !empty($course['CourseGroup']) ? htmlspecialchars($course['CourseGroup'], ENT_QUOTES, 'UTF-8') : '';
        $courseColour = !empty($course['CourseColour']) ? htmlspecialchars($course['CourseColour'], ENT_QUOTES, 'UTF-8') : '#007bff';
        ...
        print('<div class="courseHeaderContent" style="background: linear-gradient(to right, ' . $courseColour . '15, #f8f9fa);">');
        print('<div>');
        ...
        print('<a href="../CoursesAndTasks/assignCourseToUsersPage.php?courseID=' . $courseID . '" class="assignButton" onclick="event.stopPropagation();">Edit Assignees</a>');
        ...
        if ($enrollmentCount === 0) {
            ...
            foreach ($course['enrollments'] as $enrollment) {
                ...
                if (!empty($enrollmentDate) && $enrollmentDate !== '0000-00-00' && $enrollmentDate !== '0000-00-00 00:00:00') {
                    $enrolledDateFormatted = date('M j, Y', strtotime($enrollmentDate));
                }
                
                $taskCountClass = ($completedTasks === $totalTasks && $totalTasks > 0) ? 'taskCount completed' : 'taskCount';
                ...
                print('<td>');
                if (!empty($completionDate) && $completionDate !== '0000-00-00' && $completionDate !== '0000-00-00 00:00:00') {
