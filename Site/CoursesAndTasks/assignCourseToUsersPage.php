...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['assignCourseButton'])) {
  $selectedUsers = $_POST['selectedUsers'] ?? array();
  ...
  try {
    ...
    $tasksQuery = "SELECT CTTaskID FROM CourseTasksDB WHERE CTCourseID = ? ORDER BY CTTaskOrder";
    ...
    if (count($usersToAssign) > 0) {
      $insertQuery = "INSERT INTO UserTasksDB (UTUsersID, UserSetTaskID, UserSetCourseID, UserSetCourseDate, UserSetTaskComplete) VALUES (?, ?, ?, ?, 0)";
      ...
      foreach ($usersToAssign as $userId) {
        ...
        foreach ($taskIDs as $taskId) {
          ...
          $checkQuery = "SELECT COUNT(*) as RecordCount FROM UserTasksDB WHERE UTUsersID = ? AND UserSetTaskID = ?";
    ...
    if (count($usersToUnassign) > 0) {
      $deleteQuery = "DELETE FROM UserTasksDB WHERE UTUsersID = ? AND UserSetCourseID = ? AND UserSetTaskID IN ($placeholders)";
      ...
      foreach ($usersToUnassign as $userId) {
        ...
        $deleteParams = array_merge(array($userId, $courseForThisPageID), $taskIDs);
    ...
    $feedbackMessage = "<p style=\"color: red; font-weight: bold;\">Error: " . htmlspecialchars($e->getMessage(), ENT_QUOTES, 'UTF-8') . "</p>";
    $inputError = true;
  }
...
$queryUsers = "SELECT UsersID, FirstName, LastName, Email, SchoolStatus FROM UsersDB ORDER BY SchoolStatus, FirstName, LastName";
...
$usersWithCourse = array();
$userTaskCompletionStatus = array(); // Track completion status per user

if (count($taskIDs) > 0 && count($usersArray) > 0) {
  // First, get all users who have ANY task from this course
  $placeholders = implode(',', array_fill(0, count($taskIDs), '?'));
  $queryAssigned = "SELECT DISTINCT UTUsersID FROM UserTasksDB WHERE UserSetTaskID IN ($placeholders) AND UserSetCourseID = ?";
  ...
  if (count($usersWithCourse) > 0) {
    foreach ($usersWithCourse as $userId) {
      ...
      // Build parameters: userId, courseId, then all taskIds
      $completionParams = array_merge(array($userId, $courseForThisPageID), $taskIDs);
      ...
      $userTaskCompletionStatus[$userId] = array(
...
<div class=\"formContainer\">
  <h3>Tasks in this Course (" . count($tasksArray) . ")</h3>";

if (count($tasksArray) > 0) {
  $tasksListHTML .= "<p style=\"color: #666; font-size: 14px; margin-bottom: 15px;\"><em>These tasks will be assigned to all selected users:</em></p>";
...
$usersWithThisCourse = array();
$usersWithoutThisCourse = array();
...
if (count($usersWithoutThisCourse) > 0) {
  $usersListHTML .= "
      <h4 style=\"margin-top: 0; margin-bottom: 10px; color: #333;\">Users Not Yet Assigned (" . count($usersWithoutThisCourse) . ")</h4>
...
if (count($usersWithThisCourse) > 0) {
  $usersListHTML .= "
      <h4 style=\"margin-top: 20px; margin-bottom: 10px; color: #333;\">Users Already Assigned (" . count($usersWithThisCourse) . ")</h4>
...
if (count($usersArray) === 0) {
  $usersListHTML .= "<p style=\"padding: 20px; text-align: center; color: #666;\">No active users found in the system.</p>";
}
