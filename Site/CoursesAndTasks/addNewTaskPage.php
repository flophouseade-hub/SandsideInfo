...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["insertNewTaskButton"])) {
	...
	if ($inputError === false) {
		...
		if ($result->num_rows > 0) {
			...
			// Insert new task
			$insertQuery =
				"INSERT INTO tasks_tb (TaskName, TaskDescription, TaskResource, TaskGroup, TaskColour, TaskMadeBy, TaskMadeTime) VALUES (?, ?, ?, ?, ?, ?, ?)";
...
if (!empty($feedbackMessage)) {
	$feedbackMessage = "<div class=\"formBlueInfoBox\">$feedbackMessage</div>";
}
...
if ($taskAddedSuccess === true) {
	...
	if (!empty($feedbackMessage)) {
		...
		$feedbackMessage =
			"<div class=\"formBlueInfoBox\"><p>Please fill out the form below to add a new task.</p></div>";
	}

	print "
    <div class=\"formInfoBox\">
        <p>The task will be automatically tagged with your name and the current date/time.</p>
    </div>
    $feedbackMessage
