...
if (isset($_GET["deleteStudentID"]) && is_numeric($_GET["deleteStudentID"])) {
	...
	if (!$connection) {
		...
		if ($stmt->execute()) {
			...
			$feedbackMessage =
				"<p style='color: red;'>Error deleting student: " .
...
$classesQuery = "SELECT ClassID, classname FROM classes_tb ORDER BY classOrder, classname";
...
$students_tbQuery = "SELECT s.StudentID, s.FirstName, s.LastName, s.UPN, s.Sex, s.ClassID, c.classname, c.colour, c.classOrder 
                  FROM students_tb s 
                  LEFT JOIN classes_tb c ON s.ClassID = c.ClassID 
                  WHERE 1=1";
...
if (!empty($filterSex)) {
	$filterParams .= "&filterSex=" . urlencode($filterSex);
}
...
if (count($students_tb) > 0) {
	...
	print "<p style='color: #999; font-style: italic; font-size: 1.1em;'>No students_tb found" .
		($filterClass > 0 ? " in this class" : "") .
		".</p>";
	print "</div>";
}
