...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['uploadFileToSiteButton'])) {
  ...
  if ($uploadOk == 1) {
    ...
    if ($stmt->execute()) {
      ...
      $feedbackMessage .= "<p class='formFeedbackSuccess'>New Image ID: <strong>" . htmlspecialchars($newImageID, ENT_QUOTES, 'UTF-8') . "</strong></p>";
      $feedbackMessage .= "<p>Would you like to upload another?</p>";
      // Clear form values on success
      $newImageCaption = "";
      $newImageDescription = "";
    } else {
      $feedbackMessage .= "<p class='formFeedbackError'>✗ Error saving image details: " . htmlspecialchars($stmt->error, ENT_QUOTES, 'UTF-8') . "</p>";
      $uploadOk = 0;
      $inputError = true;
    }
    $stmt->close();
