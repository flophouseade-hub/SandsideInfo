...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['updateImageButton'])) {
  ...
  if ($inputOK === true) {
    ...
    if ($stmt->execute()) {
      ...
      $feedbackMessage = "<p style=\"color: red;\"><strong>ERROR: Could not update image details: " . $stmt->error . "</strong></p>";
      $inputOK = false;
    }
  ...
  <div class=\"formContainer\">
    ...
    <div class=\"formField\">
      ...
      <select name=\"fvImageGroupExisting\" id=\"fvImageGroupExisting\" class=\"formSelect\" onchange=\"handleImageGroupSelection()\">
        ...
        . implode('', array_map(function($group) use ($imageGroup) {
        ...
        "<option value=\"_new_\"" . (empty($imageGroup) || in_array($imageGroup, $imageGroups) ? '' : ' selected') . ">+ Create New Group</option>"
      . "</select>
      <span class=\"formInputHelper\">Choose an existing group or create a new one below (optional)</span>
    </div>
    
    <div class=\"formField\" id=\"newImageGroupField\" style=\"display: " . (empty($imageGroup) || in_array($imageGroup, $imageGroups) ? 'none' : 'block') . ";\">
      <label>New Group Name</label>
      <input type=\"text\" name=\"fvImageGroup\" id=\"fvImageGroup\" value=\"" . (in_array($imageGroup, $imageGroups) ? '' : $imageGroupSafe) . "\" 
