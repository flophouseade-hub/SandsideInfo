...
$groupQuery =
	"SELECT DISTINCT LRGroup FROM resource_library_tb WHERE LRGroup IS NOT NULL AND LRGroup != '' ORDER BY LRGroup ASC";
...
$_SESSION["resource_library_tb"] = [];
while ($row = mysqli_fetch_assoc($result)) {
	$_SESSION["resource_library_tb"][] = $row;
}

$resource_library_tbArray = $_SESSION["resourceLibrary"];
...
foreach ($resource_library_tbArray as $documentRef) {
	...
	$canEdit =
		$_SESSION["currentUserLogOnStatus"] == "fullAdmin" || $_SESSION["currentUserLogOnStatus"] == "pageEditor";
